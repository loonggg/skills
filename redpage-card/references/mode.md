...
用日式漫画的视觉语言讲故事——分格节奏、黑白对比、集中线和留白来制造戏剧性。纯黑做主色，赤红（`--accent: #C41E1E`）作为弹点色，制造视觉冲击。力量来自墨色浓淡、红色点缀和构图的紧张感。

## 步骤 1：读取模板

Read `~/.claude/skills/redpage-card/assets/template.html`
...
从内容中提取所有关键观点（**有几个提取几个，不限制数量**）。

提取标准：
- **独立性**：每个观点能脱离原文单独成立
- **明确性**：观点标题 ≤15 字，是一个清晰的命题
- **有据性**：观点有原文中的事实、数据或引用支撑
- **不重叠**：观点之间不重复
- **完整性**：所有观点合起来覆盖原文核心内容

### 2.2 确定卡片数量

**一个观点一张卡片**。有几个观点就生成几张。每张卡片：
- 有独立标题、独立分格、独立叙事弧
- 右上角标注 `01/N` ~ `N/N`
- 首卡破题，末卡收束
...
**系列一致性规则（同源多卡）**：当从同一个文本源生成多张卡片时，必须对整个文本源做一次整体判断，选定一个风格，所有卡片统一使用。不要按单个观点逐张选风格——风格跳跃会破坏阅读连续感。

## 步骤 3：设计画面

### 3.1 漫画视觉元素工具箱

**所有元素用 CSS + SVG 实现。黑白为主，赤红弹点为辅，不用其他彩色。**
...
**核心约束**：
- 配色限于黑(`--ink`)、白(`--white/--bg`)、灰(`--tone`)、赤红(`--accent`)四值，偶尔用 `--ink-mid`
- 赤红只用在 2-3 个关键词上，不要大面积填充
- 边框统一 2.5-3px，模拟笔触
...
写入：`/tmp/redpage_{name}_{N}.html`（N 为 01, 02, ...）
